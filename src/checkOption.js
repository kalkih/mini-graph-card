...
const checkEntities = (configEntities) => {
  ...
  configEntities.forEach((entityConfig, index) => {
    const hasEntity = entityConfig && typeof entityConfig.entity === 'string' && entityConfig.entity.trim() !== '';
    const hasStaticValue = entityConfig && entityConfig.static_value !== undefined && isNumeric(entityConfig.static_value);
